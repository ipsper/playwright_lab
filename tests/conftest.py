...
def pytest_addoption(parser):
    ...
    parser.addoption("--play_ip", action="store", default="127.0.0.1", help="playwright IP for the tests")
    parser.addoption("--play_port", action="store", default="8000", help="playwright port for the tests")
    parser.addoption("--browsers", action="store", default="", help="Comma-separated list av browsers")
    parser.addoption("--sut_port", action="store", default="8000", help="System under test port for the tests")
    parser.addoption("--sut_ip", action="store", default="127.0.0.1", help="System under test IP for the tests")
    parser.addoption("--endpoint", action="store", type=str, default="", help="entpoint on playwright for the tests")
    parser.addoption("--protocol", action="store", type=str, default="", help="System under test protocol http / https")
...
@pytest.fixture(scope='session')
def play_ip(request):
    client_typ_value = request.config.option.play_ip
    if client_typ_value is None:
        pytest.skip()
    return client_typ_value

@pytest.fixture(scope='session')
def play_port(request):
    client_typ_value = request.config.option.play_port
    if client_typ_value is None:
        pytest.skip()
    return client_typ_value
...
def browsers(request):
    arg = request.config.option.browsers
    if arg is None:
        pytest.skip()
    return arg.split(",") if arg else []

@pytest.fixture(scope='session')
def sut_port(request):
    client_typ_value = request.config.option.sut_port
    if client_typ_value is None:
        pytest.skip()
    return client_typ_value
...
@pytest.fixture(scope='session')
def sut_ip(request):
    client_typ_value = request.config.option.sut_ip
    if client_typ_value is None:
        pytest.skip()
    return client_typ_value
